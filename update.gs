const openUpdate = () => {
  ...
  let logsData = [];
  if (logsLast != 1) {
    logsData = JSON.stringify(logs.getRange(`C2:C${logsLast}`).getValues()).replace(/\[/g, "").replace(/\]/g, "").replace(/"/g, "").split(",");
  }
...
const Update = (orderNo, orderLists) => {
  ...
  if (orderLists.indexOf(orderNo) == -1) {
    showAlart("Please enter a valid Order No.");
  ...
  finalData_.forEach((v, i) => {
    if (v == -1) {
      ...
      if (backgrounds.indexOf("#6aa84f") >= 0 || backgrounds.indexOf("#00ffff") >= 0) {
        ...
        let p = (pickupDate + 3) - 5;
        if(p <= 3){
          p = 4;
        }
        let len = returnDate - p + 1 + 5;
        let range = inventoryOrder.getRange(inventoryOrderIndex + 2, p, 1, (returnDate - p + 1 + 5 + 3));
        let backgrounds = range.getBackgrounds()[0];
        let data = range.getDisplayValues()[0];
        tooltipData.push([backgrounds, data,p, (pickupDate + 3), (returnDate - pickupDate + 1)])
      }
      else {
  ...
  // try {
    ...
    for(let i = 0; i < itemsNo; i++){
      ...
      if(oldPickupDate >= 0 & oldReturnDate >= 0){
        inventoryOrder.getRange(inventoryOrderIndex + 2, (oldPickupDate + 3), 1, (oldReturnDate - oldPickupDate + 1)).setBackgrounds([blankColor]).setValues([blankData]);
      }
    }
